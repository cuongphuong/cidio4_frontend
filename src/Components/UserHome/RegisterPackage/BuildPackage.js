...
import ItemOrder from '../../HomeAdmin/Service/Pakage/ThucDon/ItemOrder';
import ItemShow from '../../HomeAdmin/Service/Pakage/ThucDon/ItemShow';
...
class BuildPackage extends Component {
    ...
    handleSubmitPackage() {
        ...
        if (obj.thucdon.length >= 5) {
            //cache cookie
             cookie.save('cidiopackage', this.b64EncodeUnicode(JSON.stringify(this.props.StatePackage)), { path: '/' });
             window.location = '/register/end';
            // console.log(this.b64DecodeUnicode(cookie.load('cidiopackage')));
                        ...
                        <div className="col-md-12">
                            <div style={{ marginBottom: '10px' }}>
                                <button onClick={() => this.handleSubmitPackage()} type="button" className="btn btn-info btn-fw">
                                    <i className="mdi mdi-upload"></i>Hoàn tất đăng kí</button><span style={{ marginLeft: '10px' }}>Build gói dịch vụ</span>
                            </div>
...
export default connect(function (state) {
    return { StateInfoRegisterService: state.StateInfoRegisterService, StatePackage: state.StatePackage, StateInfoSystem: state.StateInfoSystem, StateInfoUser: state.StateInfoUser }
})(BuildPackage);